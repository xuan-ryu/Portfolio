...
type WordSpring = { x: number; y: number; vx: number; vy: number }
...
export default function DigitalLandscape(props: Props) {
    ...
    // ── Ink cursor trail (Canvas 2D, no Three.js) ─────────────────────────────
    useEffect(() => {
        if (typeof window === "undefined") return
        ...
        // Spring follower — lags slightly behind cursor for organic ink feel
        let followerX = -9999, followerY = -9999
        let targetX = -9999, targetY = -9999
        let prevFX = -9999, prevFY = -9999
        ...
        function render(now: number) {
            ...
            // Past hero zone: drain remaining dots then stop drawing
            if (scrollProgressRef.current > 0.22) {
                if (dots.length === 0) return
                ctx!.clearRect(0, 0, canvasW, canvasH)
                for (let i = dots.length - 1; i >= 0; i--) {
                    const d = dots[i]
                    d.a *= 0.82
                    if (d.a < 0.008) { dots.splice(i, 1); continue }
                    drawDot(d)
                }
                return
            }
            ...
            prevFX = followerX
            prevFY = followerY
            followerX += (targetX - followerX) * 0.20
            followerY += (targetY - followerY) * 0.20

            const dx = followerX - prevFX
            const dy = followerY - prevFY
            const speed = Math.sqrt(dx * dx + dy * dy)

            // Spawn ink blobs at follower position (one burst per ~18ms frame)
            if (speed > 0.4 && now - lastSpawn > 14 && followerX !== -9999) {
                lastSpawn = now
                const count = Math.min(Math.ceil(speed / 5), 4)
                for (let i = 0; i < count; i++) {
                    if (dots.length >= MAX_DOTS) dots.shift()
                    const jAngle = Math.random() * Math.PI * 2
                    const jLen = Math.random() * speed * 0.12
                    dots.push({
                        x: followerX + Math.cos(jAngle) * jLen,
                        y: followerY + Math.sin(jAngle) * jLen,
                        vx: dx * (0.08 + Math.random() * 0.14),
                        vy: dy * (0.08 + Math.random() * 0.14),
                        r: 1.8 + Math.random() * Math.min(speed * 0.16, 6),
                        a: 0.16 + Math.random() * 0.20,
                    })
                }
            }

            // Integrate physics + decay
            for (let i = dots.length - 1; i >= 0; i--) {
                const d = dots[i]
                d.vy += 0.022   // subtle gravity
                d.vx *= 0.955
                d.vy *= 0.955
                d.x += d.vx
                d.y += d.vy
                d.r  *= 0.984
                d.a  *= 0.935
                if (d.a < 0.008 || d.r < 0.22) { dots.splice(i, 1); continue }
            }
        ...
        function drawDot(d: InkDot) {
            const spread = d.r * 1.9
            const grd = ctx!.createRadialGradient(d.x, d.y, 0, d.x, d.y, spread)
            grd.addColorStop(0,    `rgba(22, 14, 6, ${d.a})`)
            grd.addColorStop(0.42, `rgba(16, 10, 4, ${d.a * 0.52})`)
            grd.addColorStop(1,    `rgba(8,  5,  2, 0)`)
            ctx!.beginPath()
    ...
    // ── Magnetic word repulsion (spring physics, pure JS, no lib) ────────────
    useEffect(() => {
        if (typeof window === "undefined") return

        const RADIUS   = 140   // px — repulsion field radius
        const STRENGTH = 24    // push force magnitude
        const DAMPING  = 0.72  // per-frame velocity decay
        const RETURN   = 0.06  // spring return toward origin

        const springs: WordSpring[] = [
            { x: 0, y: 0, vx: 0, vy: 0 },
            { x: 0, y: 0, vx: 0, vy: 0 },
            { x: 0, y: 0, vx: 0, vy: 0 },
        ]
        ...
        // Cache rects — only invalidate on resize
        let cachedRects: (DOMRect | null)[] = [null, null, null]
        const refreshRects = () => {
            cachedRects = wordRefs.current.map(el =>
        ...
        function loop() {
            ...
            // Skip magnetic effect once user scrolls into page 2
            ...
            for (let i = 0; i < springs.length; i++) {
                ...
                if (Math.abs(s.x) > 0.06 || Math.abs(s.vx) > 0.01 ||
                    Math.abs(s.y) > 0.06 || Math.abs(s.vy) > 0.01) {
                    anyMotion = true
                }
            }

            if (anyMotion || mouseX !== -9999) {
                rafId = requestAnimationFrame(loop)
            } else {
                active = false
            }
    ...
    return (
        ...
        >
            ...
            *, *::before, *::after { -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; box-sizing: border-box; }
            ...
            .scene-loaded .framer-xy-hero .w-mag:nth-child(1) .w { animation-delay: 0.00s; }
            .scene-loaded .framer-xy-hero .w-mag:nth-child(2) .w { animation-delay: 0.10s; }
            .scene-loaded .framer-xy-hero .w-mag:nth-child(3) .w { animation-delay: 0.22s; }
            ...
            /* ── Magnetic word wrappers — JS spring sets translate3d here ── */
            .framer-xy-hero .w-mag {
                display: inline-block;
                will-change: transform;
            }
            /* Ink canvas overlay */
...
`,
            ...
            >
                ...
                {/* Ink cursor trail canvas — sits above Three.js, below text UI */}
                <canvas
                    ref={inkCanvasRef}
                    className="ink-trail-canvas"
                />
                ...
                >
                    ...
                    >
                        ...
                        >
                            <span className="w-mag" ref={el => { wordRefs.current[0] = el }}>
                                <span className="w">{heroWord1}</span>
                            </span>
                            <span className="w-mag" ref={el => { wordRefs.current[1] = el }}>
                                <span className="w">{heroWord2}</span>
                            </span>
                            <span className="w-mag" ref={el => { wordRefs.current[2] = el }}>
...
DigitalLandscape.defaultProps = {
    ...
    page2Footer: "Vibe coded — not engineered.\nBuilt in conversation with the machine,\nfelt before it was deployed.",
}
